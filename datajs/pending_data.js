...
function load_more() {
	...
	var url=BaseURL+"/"+dir+"/phonegp/list_orders.php?delivery_boy_id="+delivery_boy_id+"&status=pending&offset="+v ; // alert(url);
...
*/			
	...
	var url=BaseURL+"/"+dir+"/phonegp/list_orders.php?delivery_boy_id="+delivery_boy_id+"&status=pending" ;  //alert(url);
		...
		$.getJSON(url,function(result){ //alert(result);
	        ...
	        $.each(result, function(i, field){
				...
				if(i==0){
					...
					$("#next_off").val(next_offset);
					$("#tot_cnt").html(tot_cnt_data); //alert(tot_cnt_data);
				} else {
	        	var customer_name=field.customer_name;
				var customer_phone=field.customer_phone;
				var order_date=field.order_date;
